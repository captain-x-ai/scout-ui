...
export function Clip({ t, ar, r, onAccept, onConfirm, onRegen, busy }) {
  ...
  const aiAnalyzing = pending && mediaReady && !reviewReady;
  ...
  return (
    <>
      {preview && cachedMedia?.videoUrl && (
        <ClipPreviewModal t={t} clip={previewClip} onClose={() => setPreview(false)} />
      )}
      <div className={`panel ${pending ? "" : ""}`} style={{ padding: 18, opacity: pending ? .92 : 1, borderColor: overruled ? "rgba(242,128,46,.4)" : "var(--line2)" }}>
        <div style={{ display: "flex", gap: 14 }}>
          <div style={{ width: 92, flexShrink: 0 }}>
            ...
            >
              ...
              <span className="mono" style={{
                position: "absolute", bottom: 5, insetInlineEnd: 6, fontSize: 9.5,
                color: "var(--cream)", background: "rgba(0,0,0,.55)", padding: "1px 5px", borderRadius: 4, zIndex: 1,
              }}>{r.minute}'</span>
            </div>
            {mediaError && (
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {reviewReady || aiAnalyzing || videoProcessing ? (
              <div style={{
