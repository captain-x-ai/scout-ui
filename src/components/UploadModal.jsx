...
export function UploadModal({ t, ar, player, onClose, onAdd, locked, closable, busy, statusMsg }) {
  ...
  return (
    <div className="overlay no-print" onClick={closable ? handleClose : undefined}>
      <div className="panel modal scrolly" style={{ padding: 24, maxWidth: 520 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr .55fr 1fr", gap: 8, marginBottom: 8 }}>
            ...
            <input className="fld" placeholder={t.clipOpp} value={clip.opp} onChange={(e) => set("opp", e.target.value)}
              style={{ padding: "9px 11px", fontSize: 13 }} disabled={fieldsDisabled} />
            <input className="fld" placeholder={t.clipMin} type="number" value={clip.minute} onChange={(e) => set("minute", e.target.value)}
              style={{ padding: "9px 11px", fontSize: 13 }} disabled={fieldsDisabled} />
