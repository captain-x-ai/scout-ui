...
export function Report({ t, ar, player, players, setSel, onBack }) {
    ...
    <div className="fade">
      ...
      <div className="panel" style={{ padding: 0, overflow: "hidden", maxWidth: 760, margin: "0 auto" }}>
        ...
        <div style={{ padding: "26px 30px", background: "var(--primary)", position: "relative", overflow: "hidden", borderBottom: "1px solid var(--accent)" }}>
          ...
          <div style={{ position: "relative", zIndex: 1, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
            <div>
              ...
              <div className="serif" style={{ fontSize: 32, fontWeight: 800, marginTop: 4 }}>{ar ? player.nameAr : player.name}</div>
              <div style={{ color: "rgba(244,242,251,.7)", fontSize: 13, marginTop: 5 }}>{player.club} · {player.pos} · {t.age} {player.age} · {player.value}</div>
            </div>
            <div style={{ textAlign: ar ? "left" : "right" }}>
              ...
              <div style={{ fontSize: 10.5, color: "rgba(244,242,251,.65)" }}>{t.asOf} 04 Jun 2026</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: 30 }}>
          ...
          <div style={{ marginBottom: 26 }}>
            {Object.entries(player.attrs).map(([k, v]) => <Bar key={k} label={t[k]} v={v} />)}
          </div>
